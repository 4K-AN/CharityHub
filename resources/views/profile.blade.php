...
<main id="profile-content" class="hidden flex-1 max-w-container-max-width mx-auto px-margin-mobile md:px-margin-desktop py-10 w-full">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        ] as [$id, $default, $label, $icon, $bgGrad, $textColor])
        <div class="bg-surface-container-lowest rounded-2xl border border-outline-variant/10 p-6 shadow-soft card-hover reveal">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 rounded-xl bg-gradient-to-br {{ $bgGrad }} flex items-center justify-center shrink-0">
                    <span class="material-symbols-outlined text-[24px] {{ $textColor }}">{{ $icon }}</span>
                </div>
                <div>
                    <p class="text-label-sm text-on-surface-variant">{{ $label }}</p>
                    <p id="{{ $id }}" class="text-headline-md {{ $textColor }} font-bold">{{ $default }}</p>
                </div>
            </div>
        </div>
        @endforeach
